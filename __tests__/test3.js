...
describe('Incorrect Button Click Causes Game Reset', () => {
    let startButton;
    let levelText;
    let statusText;
    ...
    test('Should reset game to level 1 on incorrect button click', () => {
      // Simulate starting the game
      startButton.click();
  
      // Simulate the user clicking the wrong button (assuming the sequence is [0] and the wrong one clicked is [1])
      const wrongButton = document.querySelector('button[data-key="1"]');
      wrongButton.click();
  
      // Check if the game is reset
      expect(levelText.textContent).toContain('Current Level: 1');
      expect(statusText.textContent).toBe('Game Over!! Click Start to restart the game!');
    });
  });
